...
export class VehiclesResolver {
    ...
    @Mutation(() => Vehicule, { name: 'createVehicle', description: 'Ajouter un nouveau véhicule' })
    createVehicle(@Args('createVehicleInput') createVehicleInput: CreateVehicleInput): Promise<Vehicule> {
        return this.vehiclesService.createVehicle(createVehicleInput);
    }

    @Mutation(() => VehiclePosition, { name: 'addPosition', description: 'Enregistrer une position GPS' })
    addPosition(@Args('createPositionInput') createPositionInput: CreatePositionInput): Promise<VehiclePosition> {
    ...
    @Query(() => [Vehicule], { name: 'vehicles', description: 'Consulter la liste des véhicules' })
    findAllVehicles(): Promise<Vehicule[]> {
        return this.vehiclesService.findAllVehicles();
    }

    @Query(() => Vehicule, { name: 'vehicle', description: 'Consulter le détail d\'un véhicule' })
    findOneVehicle(@Args('id', { type: () => Int }) id: number): Promise<Vehicule> {
    ...
    updateVehicle(@Args('updateVehicleInput') updateVehicleInput: UpdateVehicleInput): Promise<Vehicule> {
        return this.vehiclesService.updateVehicle(updateVehicleInput.id, updateVehicleInput);
    }
    ...
    updateVehicleStatus(@Args('updateVehicleStatusInput') updateVehicleStatusInput: UpdateVehicleStatusInput): Promise<Vehicule> {
    ...
    @ResolveReference()
    resolveReference(reference: { __typename: string; id: number }): Promise<Vehicule> {
